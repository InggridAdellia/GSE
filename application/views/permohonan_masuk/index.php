<?php
function item_lulus_tahap($item, $tahap, $urutan_tahap) {
    $idx_batas = array_search($tahap, $urutan_tahap);
    $idx_item  = array_search($item->tahap_saat_ini ?? 'operasi', $urutan_tahap);
    return $idx_item !== false && $idx_batas !== false && $idx_item > $idx_batas;
}

$semua_masuk = $semua_masuk ?? [];
$kontrak    = $kontrak    ?? [];
...
$all          = array_merge($semua_masuk, $kontrak);
$total_p      = count($all);
$count_setuju = 0;
$count_tolak  = 0;
foreach ($all as $row) {
    if ($row->status === 'Disetujui') $count_setuju++;
    if ($row->status === 'Ditolak')   $count_tolak++;
}
...
<div class="page-head">
    <div>
        <h5 class="page-title"><i class="bi bi-envelope-arrow-down"></i> Pengajuan Permohonan Izin Masuk GSE</h5>
        <p class="page-subtitle">Daftar permohonan izin masuknya unit GSE ke area bandara.</p>
    </div>
    <?php if ($is_gh): ?>
    <a href="<?= site_url('permohonan_masuk/tambah') ?>" class="btn btn-primary btn-sm">
        <i class="bi bi-plus-lg"></i> Buat Permohonan Masuk
    </a>
    <?php endif; ?>
</div>
...
<div class="jenis-tabs">
    <button class="jenis-tab active" onclick="switchTab('semua_masuk', this)">
        <i class="bi bi-box-arrow-in-right"></i> Permohonan Masuk
        <span class="pill"><?= count($semua_masuk) ?></span>
    </button>
    <button class="jenis-tab" onclick="switchTab('kontrak', this)">
        <i class="bi bi-arrow-repeat"></i> Perbaruan Kontrak
        <span class="pill"><?= count($kontrak) ?></span>
    </button>
</div>

<!-- Tab: Permohonan Masuk -->
<div id="tab-semua_masuk" class="tab-pane-custom show">
    <?php echo render_table_masuk($semua_masuk, 'Permohonan Masuk', $badge, $is_gh); ?>
</div>

<!-- Tab: Perbaruan Kontrak -->
<div id="tab-kontrak" class="tab-pane-custom">
    <?php echo render_table_masuk($kontrak, 'Perbaruan Kontrak', $badge, $is_gh); ?>
</div>
...
<?php
function render_table_masuk($rows, $jenis_label, $badge, $is_gh) {
    ob_start();
    ?>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="7" class="p-0">
                                <div class="empty-state">
                                    ...
                                    <?php if ($is_gh): ?>
                                    <p class="text-muted small mb-0">Klik "Buat Permohonan Masuk" untuk mengajukan yang baru.</p>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($jenis_label === 'Perbaruan Kontrak'): ?>
                                    <?php if (!empty($row->daftar_gse)): ?>
                                        <?php foreach ($row->daftar_gse as $g): ?>
                                        <div class="small text-muted"><?= htmlspecialchars($g->no_asset ?? '-') ?></div>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <span class="text-muted">-</span>
                                    <?php endif; ?>
                                ...
                                ?>
                                <?php if ($jenis_label !== 'Perbaruan Kontrak' && $jml_total > 0): ?>
                                    <?php if ($is_gh): ?>
                                    <a href="<?= site_url('permohonan_masuk/kelengkapan/' . $row->id_permohonan_masuk) ?>"
                            ...
                            <td style="min-width:190px;">
                                <?php
                                    ...
                                    $status_cls   = $cls;
                                    if ($jenis_label !== 'Perbaruan Kontrak' && $jml_total > 0 && !$sdh_lengkap
                                        && $row->status === 'Menunggu Verifikasi Operasi') {
                                        $status_label = 'Menunggu Lengkapi Berkas';
                                        $status_cls   = 'bg-secondary';
                                    }
                                ?>
                                <span class="badge <?= $status_cls ?>"><?= htmlspecialchars($status_label) ?></span>
                                <?php if ($row->status === 'Ditolak' && $row->alasan_penolakan): ?>
                                <div class="reject-hint mt-1">
                                    <i class="bi bi-info-circle"></i>
                                    <?= htmlspecialchars(mb_strimwidth($row->alasan_penolakan, 0, 50, '...')) ?>
                                </div>
                                <?php endif; ?>
                                <div class="mt-1 d-flex gap-1 flex-wrap">
                                    <?php
                                    $is_pms = ($row->jenis_permohonan === 'Masuk Perbaikan Sparepart' || $row->jenis_permohonan === 'Sparepart');
                                    if ($is_pms) {
                                        $tahap_pills = [
                                            'Ops' => 'operasi',
                                            'Sec' => 'security',
                                    ...
                                    foreach ($tahap_pills as $label => $kode):
                            ...
                            <td class="text-end">
                                <div class="row-actions">
                                    <a href="<?= site_url('permohonan_masuk/detail/' . $row->id_permohonan_masuk) ?>"
                                    ...
                                    <?php if ($is_gh): ?>
                                        ...
                                        <a href="<?= site_url('permohonan_masuk/hapus/' . $row->id_permohonan_masuk) ?>"
                                           class="btn btn-sm btn-outline-danger" title="Hapus"
                                           onclick="return confirm('Hapus permohonan masuk ini?')">
