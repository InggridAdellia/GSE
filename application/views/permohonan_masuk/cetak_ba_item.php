...
<head>
    <meta charset="UTF-8">
    <title>Berita Acara - <?= htmlspecialchars($gse->nama_gse) ?></title>
...
<div style="text-align:left; margin-bottom:10px;">
    <img src="<?= base_url('logo_injourney.png') ?>" alt="Injourney Airports" style="max-height:90px;">
</div>

<?php
$is_sparepart_ba = (($gse->jenis_item ?? '') === 'Sparepart' || strtolower($gse->manufacture_type ?? '') === 'sparepart' || ($permohonan->jenis_permohonan ?? '') === 'Masuk Perbaikan Sparepart');
?>
...
<?php
$tgl_acuan = $gse->verifikasi_at ?? $permohonan->updated_at;

$tgl     = date('d', strtotime($tgl_acuan));
$bulan   = date('F', strtotime($tgl_acuan));
$bulanId = [
...
$tahun     = date('Y', strtotime($tgl_acuan));
$hariId    = [
    'Sunday'=>'Minggu','Monday'=>'Senin','Tuesday'=>'Selasa',
    'Wednesday'=>'Rabu','Thursday'=>'Kamis','Friday'=>'Jumat','Saturday'=>'Sabtu'
];
$hari = $hariId[date('l', strtotime($tgl_acuan))] ?? '';
?>
...
var bc_ops      = <?= json_encode($fmt($nama_operasi,   $gse->verifikasi_at ?? $permohonan->updated_at)) ?>;
var bc_security = <?= json_encode($fmt($nama_security,  $gse->verifikasi_at ?? $permohonan->updated_at)) ?>;
...
<?php if (!$is_sparepart_ba): ?>
var bc_sales    = <?= json_encode($fmt($nama_sales,     $gse->verifikasi_at ?? $permohonan->updated_at)) ?>;
